...
void loop() {
  ...
  if (distance < _DIST_MIN) {
    distance = distance;    // Set Lower Value
    digitalWrite(PIN_LED, 255);       // LED OFF
  } else if (distance > _DIST_MAX) {
    distance = distance;    // Set Higher Value
    digitalWrite(PIN_LED, 255);       // LED OFF
  } else if (distance == 200) {
    analogWrite(PIN_LED, 0);
  } else if (distance > 200) {
    analogWrite(PIN_LED, 255 - (distance - 200) * 255/100);
  } else if (distance < 200) {
    analogWrite(PIN_LED, (distance - 100) * 255/100);
  } else {    // In desired Range
    digitalWrite(PIN_LED, 255);       // LED ON      
  }
